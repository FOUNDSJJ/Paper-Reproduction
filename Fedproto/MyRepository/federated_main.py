...
from utils import get_dataset, exp_details, proto_aggregation, agg_func
...
#change begin
    with open(fileName, 'a') as file:
        file.write('\nFor all users (with protos), mean of test acc is {:.5f}, std of test acc is {:.5f}'.format(
            np.mean(acc_list_g), np.std(acc_list_g)))
        file.write('\nFor all users (w/o protos), mean of test acc is {:.5f}, std of test acc is {:.5f}'.format(
            np.mean(acc_list_l), np.std(acc_list_l)))
        file.write('\nFor all users (with protos), mean of proto loss is {:.5f}, std of test acc is {:.5f}'.format(
...
#change begin
    with open(fileName, 'a') as file:
        file.write('\nFor all users (with protos), mean of test acc is {:.5f}, std of test acc is {:.5f}'.format(np.mean(acc_list_g), np.std(acc_list_g)))
